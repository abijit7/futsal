...
public class FutsalRequest {
    ...
    // No verified/rating/reviewCount here, deliberately. A venue must not be able to describe its
    // own approval status or its own reputation. Verification is PUT /api/futsals/{id}/verification
    // (admin only); rating and reviewCount are recomputed by ReviewService from the reviews table.
    // Unknown JSON properties are ignored by default, so a client still sending them is not obeyed.

...
const FutsalStore = {
  key: 'selected_futsal',
  save(futsal) {
    localStorage.setItem(this.key, JSON.stringify(futsal));
  },
  get() {
  ...
  if (user) {
    if (user.role === 'ADMIN') {
      links = `
        ...
        <a href="${basePath}pages/admin-futsals.html" class="nav-link ${activePage==='futsals'?'active':''}">Futsals</a>
      ...
      links = `
        ...
        <a href="${basePath}pages/futsals.html" class="nav-link ${activePage==='futsals'?'active':''}">Futsals</a>

...
const linkClass = ({ isActive }: { isActive: boolean }) =>
  `rounded-2xl px-4 py-2 text-sm font-bold transition ${isActive ? 'bg-green-50 text-green-700' : 'text-slate-600 hover:bg-slate-100 hover:text-slate-950'}`;
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-slate-200/80 bg-white/90 backdrop-blur-xl">
      <div className="container-page flex min-h-20 items-center justify-between gap-4">
        ...
        <button className="btn-soft px-3 py-3 lg:hidden" onClick={() => setOpen((value) => !value)} aria-label="Toggle navigation">
      ...
      {open && (
        <div className="border-t border-slate-100 bg-white lg:hidden">
          <div className="container-page flex flex-col gap-2 py-4">
            {nav.map(([label, path]) => (
              <NavLink key={path} to={path} onClick={() => setOpen(false)} className={linkClass}>{label}</NavLink>
            ))}
            {!user ? (

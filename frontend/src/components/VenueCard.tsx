  ...
  return (
    <article className="group overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl hover:shadow-slate-950/10">
      <div className="relative aspect-[4/3] overflow-hidden bg-slate-100">
        ...
        {available && (
          <div className="absolute left-4 top-4 rounded-full bg-green-600 px-3 py-1 text-xs font-semibold text-white">Available</div>
        )}
      </div>
      <div className="p-6">
        <div className="mb-3 flex items-start justify-between gap-3">
          ...
          {rated ? (
            <div className="flex shrink-0 items-center gap-1 rounded-full bg-amber-50 px-2 py-1 text-sm font-semibold tabular-nums text-amber-600">
              ...
              {(futsal.reviewCount ?? 0) > 0 && (
                <span className="font-normal text-amber-700/70">({futsal.reviewCount})</span>
              )}
            </div>
          ) : (
        ...
        <div className="mt-6 flex items-center justify-between">
          <div>
            <div className="text-xs uppercase tracking-wide text-muted">From</div>
            <div className="text-xl font-semibold tabular-nums text-slate-950">{money(futsal.hourlyPrice)}<span className="text-sm font-normal text-muted">/hr</span></div>
          </div>
          <Link to={`/venues/${futsal.futsalId}`} className="inline-flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-slate-900 text-white transition hover:bg-slate-800 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-slate-200" aria-label={`View ${futsal.name}`}>
            <ArrowRight />
          </Link>
        </div>
      </div>

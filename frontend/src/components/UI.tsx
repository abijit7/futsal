...
export function PageHero({
  ...
  return (
    <section className="mb-8 overflow-hidden rounded-[2rem] bg-slate-950 p-7 text-white shadow-xl shadow-slate-950/10 md:p-9">
...
export function Chip({ children, tone = 'slate', onRemove }: { children: ReactNode; tone?: Tone; onRemove?: () => void }) {
  return (
    <span className={`inline-flex items-center gap-2 rounded-full px-3 py-1 text-xs font-black ring-1 ${toneClasses[tone]}`}>
      {children}
      {onRemove && (
        <button type="button" aria-label="Remove filter" className="rounded-full hover:bg-black/10" onClick={onRemove}>
          <X size={13} />
        </button>
      )}
    </span>

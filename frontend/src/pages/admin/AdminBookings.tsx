...
export function AdminBookings() {
  ...
  return (
    <section>
      ...
      {loading ? <LoadingState /> : items.length === 0 ? <EmptyState title="No bookings found" /> : <div className="grid gap-4">{items.map((booking) => <div key={booking.bookingId} className="panel p-5"><div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between"><div><h3 className="font-black text-slate-950">{booking.user?.name || 'User'} · {booking.timeSlot?.futsal?.name || 'Venue'}</h3><p className="mt-1 text-sm font-bold text-slate-500">{formatDate(booking.timeSlot?.slotDate)} · {timeRange(booking.timeSlot?.startTime, booking.timeSlot?.endTime)}</p><p className="mt-1 text-xs text-slate-400">{booking.paymentMethod} {booking.paymentRef}</p></div><StatusBadge status={booking.status} /></div><div className="mt-4 flex flex-wrap gap-2"><button className="btn-soft px-4 py-2" onClick={() => update(booking.bookingId, 'APPROVED')}>Approve</button><button className="btn-soft px-4 py-2" onClick={() => update(booking.bookingId, 'REJECTED')}>Reject</button><button className="btn-soft px-4 py-2" onClick={() => update(booking.bookingId, 'CANCELLED')}>Cancel</button><button className="btn-navy px-4 py-2" onClick={() => remove(booking.bookingId)}>Delete</button></div></div>)}</div>}
